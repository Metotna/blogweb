  ...
  <div class="tic_public">
    <div class="tp_jc tp_">
      <div class="_overview _cot">
        <div class="_title">竞彩足球</div>
        <div class="_title_infor _t_15px">
          <div class="_t_i_">　订单号：{{infor.orderNo}}</div>
          <div class="_t_i_">申请时间：{{infor.applyTime}}</div>
          <!-- <div class="_t_i_">处理时间：{{infor.handleTime}}</div> -->
          <div class="_t_i_">　　用户：{{infor.user}}</div>
          <div class="_t_i_" v-if="infor.bonus==null">中奖金额：-- </div>
          <div class="_t_i_" v-else>中奖金额：{{infor.bonus}}</div>
          <div class="_t_i_">投注金额：{{infor.bet}}</div>
          <div class="_t_i_">　　倍数：{{infor.multi}}</div>
          <div class="_t_i_">　　注数：{{infor.bet/2/infor.multi}}</div>
          <div class="_t_i_">　代理商：{{infor.merName}}</div>
          <div class="_t_i_">订单状态：{{infor.ticketStatus|orderState}}</div>
          <!-- <div class="_t_i_">　　期次：{{infor.lotIssue}}</div> -->
        </div>
      </div>
      <div class="_detial _cot">
        ...
        <div class="_title_infor">
          <li class="listcont">
            <span>玩法:</span>
            <font>{{infor.contextApply|work_order_play}}</font>
          </li>
        ...
        .then(res => {
          if (res.status) {
            if (res.status == 200) {
              const data = res.data;
              this.infor = data;
              this.contextTicket = data.contextTicket;
              this.contextApply = data.contextApply;
              this.infor.contents = this.handleAys(data.contextApply, data.lotId);
            } else {
